<?php

/**
 * Componente: Sección Categorías de Eventos
 *
 * Las cards se renderizan SOLO con lo que esté en el repeater ACF (beneficios > beneficio).
 * Si hay 3 ítems en el repeater → 3 cards. No se autocompletá con términos.
 */

// ── Campos ACF ────────────────────────────────────────────────────────────────
$beneficios_group = $GLOBALS['xp_beneficios_group'] ?? null;
$beneficios = $beneficios_group['beneficio'] ?? null;

$encabezado = $GLOBALS['xp_encabezado'] ?? null;
$subtitulo = $encabezado['subtitulo'] ?? null;
$titulo = $encabezado['titulo'] ?? null;
$descripcion = $encabezado['descripcion'] ?? null;

if (empty($beneficios))
    return;
?>

<section class="py-12 lg:py-20 bg-gray-50 mb-10">
    <div class="container mx-auto px-4 lg:px-8">

        <!-- Header -->
        <?php if ($subtitulo || $titulo || $descripcion): ?>
            <div class="text-center mb-12">
                <?php if ($subtitulo): ?>
                    <p class="text-sm text-gray uppercase tracking-wider mb-4" data-aos="fade-down">
                        <?php echo esc_html($subtitulo); ?>
                    </p>
                <?php endif; ?>

                <?php if ($titulo): ?>
                    <h2 class="text-3xl lg:text-4xl font-semibold text-primary" data-aos="fade-up">
                        <?php echo esc_html($titulo); ?>
                    </h2>
                <?php endif; ?>

                <?php if ($descripcion): ?>
                    <p class="mt-4 text-gray-600 max-w-2xl mx-auto" data-aos="fade-up">
                        <?php echo esc_html($descripcion); ?>
                    </p>
                <?php endif; ?>
            </div>
        <?php endif; ?>

        <!-- Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-[300px]">
            <?php foreach ($beneficios as $item):
                $imagen = $item['imagen_de_fondo'] ?? null;
                $titulo_card = $item['titulo'] ?? null;
                $boton = $item['boton'] ?? null;
                $cantidad_eventos = $item['cantidad_de_eventos'] ?? null;

                // Omitir ítem si no tiene ningún dato útil
                if (empty($imagen) && empty($titulo_card) && empty($boton) && empty($cantidad_eventos))
                    continue;

                $image_url = $imagen['url'] ?? null;
                $boton_text = $boton['title'] ?? null;
                $boton_href = $boton['url'] ?? null;
                $boton_target = $boton['target'] ?? '_self';
                ?>

                <!-- Card -->
                <div class="group relative h-full min-h-[300px] cursor-pointer rounded-[20px]
                border-[0px] border-transparent
                transition-[transform,box-shadow,border]
                hover:border-primary hover:-translate-y-2 hover:shadow-2xl" style="will-change: transform">

                    <!-- Inner Card -->
                    <div class="relative h-full rounded-[20px] overflow-hidden
                    transition-transform duration-700 ease-out">

                        <!-- Header Rojo -->
                        <div class="absolute top-0 left-0 right-0 z-20
                        -translate-y-full group-hover:translate-y-0
                        transition-transform duration-700 ease-out delay-100
                        rounded-t-2xl">
                            <div class="text-center py-2 px-2 bg-primary">
                                <?php if ($titulo_card): ?>
                                    <h3 class="text-white text-lg font-semibold mb-0">
                                        <?php echo esc_html($titulo_card); ?>
                                    </h3>
                                <?php endif; ?>

                                <?php if ($cantidad_eventos !== ''): ?>
                                    <p class="text-white/90 text-sm mb-0">
                                        <?php echo esc_html($cantidad_eventos); ?>
                                        evento<?php echo intval($cantidad_eventos) !== 1 ? 's' : ''; ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                            <div class="flex justify-between border-l-8 border-r-8 border-primary">
                                <img src="<?php echo get_template_directory_uri() . '/images/comsatel-border.png'; ?>"
                                    alt="Comsatel Border">
                                <img src="<?php echo get_template_directory_uri() . '/images/comsatel-border.png'; ?>"
                                    alt="Comsatel Border" class="rotate-90">
                            </div>
                        </div>

                        <!-- Imagen -->
                        <?php if (!empty($image_url)): ?>
                            <div class="absolute inset-0">
                                <img src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($titulo_card ?? ''); ?>"
                                    class="w-full h-full object-cover
                                transition-transform duration-1000 ease-out
                                group-hover:scale-110">

                                <!-- Overlay -->
                                <div class="absolute inset-0 bg-gradient-to-t
                                    from-black/80 via-black/20 to-transparent
                                    border-0 border-transparent rounded-[20px]
                                    transition-[border-width,border-color]
                                    duration-[2000ms]
                                    group-hover:duration-0
                                    group-hover:from-black/90
                                    group-hover:via-black/40
                                    group-hover:border-primary
                                    group-hover:border-8">
                                </div>
                            </div>
                        <?php endif; ?>

                        <!-- Badge contador -->
                        <?php if ($cantidad_eventos !== ''): ?>
                            <div class="absolute top-2 right-0 z-10
                        transition-transform duration-500 ease-in delay-100
                        group-hover:translate-x-full">
                                <span class="bg-primary text-white text-md font-medium px-4 py-2 rounded-bl-lg shadow-lg">
                                    <?php echo esc_html($cantidad_eventos); ?>
                                    evento<?php echo intval($cantidad_eventos) !== 1 ? 's' : ''; ?>
                                </span>
                            </div>
                        <?php endif; ?>

                        <!-- Contenido -->
                        <div class="absolute bottom-0 left-0 right-0 p-6 z-10">

                            <?php if ($titulo_card): ?>
                                <!-- Título -->
                                <h3 class="text-white text-xl font-semibold mb-4 leading-tight
                            transition-[opacity,transform] duration-500 ease-out
                            group-hover:opacity-0 group-hover:-translate-y-3">
                                    <?php echo esc_html($titulo_card); ?>
                                </h3>
                            <?php endif; ?>

                            <?php if (!empty($boton_href)): ?>
                                <!-- Botón -->
                                <a href="<?php echo esc_url($boton_href); ?>" target="<?php echo esc_attr($boton_target); ?>"
                                    class="btn btn-primary font-normal !text-sm inline-block
                            transition-[transform,box-shadow] duration-500 ease-out delay-150
                            group-hover:-translate-y-4 group-hover:scale-105 group-hover:shadow-xl">
                                    <?php echo esc_html($boton_text ?? 'Leer más'); ?>
                                </a>
                            <?php endif; ?>

                        </div>

                    </div>
                </div>

            <?php endforeach; ?>
        </div>
    </div>
</section>