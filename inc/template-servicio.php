...
<main class="producto-gps-page bg-gray-50">
    ...
    <!-- Hero Banner -->
    <section class="relative h-screen min-h-[600px] flex items-end bg-dark-900 <?php echo wp_title(); ?>">
    ...
    <?php
    $nav_buttons = array();
    $acf_group = get_field('servicios', 'options');
    ...
    if (empty($nav_buttons)) {
        $nav_buttons = array(
            ...
            array('label' => 'Resultados medibles', 'url' => '#resultados', 'style' => 'btn btn-outline-white', 'delay' => 600),
            array('label' => 'Preguntas frecuentes', 'url' => '#preguntas-frecuentes', 'style' => 'btn btn-outline-white', 'delay' => 700),
        );
    }
    ...
    // Construir cards solo desde ACF
    $carousel_cards = [];
    if (!empty($bs_items)) {
        $counter = 1;
    ...
    // Solo renderizar la sección si hay banner o cards
    ...
    <!-- Stats Cards Section -->
    <?php
    $acf_cards_group = get_field('cards', get_the_ID());
    $acf_card_list = $acf_cards_group['card'] ?? [];

    $cards_from_acf = [];
    foreach ($acf_card_list as $card) {
        $stat = $card['titulo'] ?? '';
        $desc = $card['descripcion'] ?? '';
        if (!$stat && !$desc)
            continue;
        $cards_from_acf[] = array(
    ...
    if (!empty($cards_from_acf)):
        get_template_part('inc/componentes/section-stats-cards', null, array(
            'section_id' => 'resultados',
            'title' => 'Resultados medibles, Confianza comprobada',
            'cards' => $cards_from_acf,
        ));
    endif;
    ?>
